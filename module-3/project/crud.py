...
def list_overdue_books(days: int = 14):
    ...
    with Session(engine) as session:
        return session.scalars(
            select(Borrowing)
            .where(Borrowing.return_date.is_(None))
            .where(Borrowing.checkout_date < cutoff)
        ).all()
...
def return_book(borrowing_id: int):
    ...
    with Session(engine) as session:
        ...
        if borrowing is None:
            raise ValueError(f"Borrowing with id {borrowing_id} not found.")
        if borrowing.return_date is not None:
            raise ValueError("This book has already been returned.")

        borrowing.return_date = date.today()
        book = session.get(Book, borrowing.book_id)
        if book is not None:
            book.available_copies += 1
...
def delete_book(book_id: int):
    ...
    with Session(engine) as session:
        ...
        if active_borrowings:
            raise ValueError("Cannot delete a book that has active borrowings.")
